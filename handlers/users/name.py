...
@dp.message_handler(state=States.enter_name)
...
@dp.message_handler(commands=["update_name"], state=States.type_msg)
async def update_handler(message: types.Message):
    ...
    await States.update_name.set()


@dp.message_handler(state=States.update_name)
async def update_name(message: types.Message):
    old_name = await get_user_name(message.from_user.id)
    is_present = await exists(user_name=message.text)
    if len(message.text) < 2 or not message.text.isalpha():
        await message.answer('Please enter a real name.')
    elif message.text == old_name:
        await message.answer("This name is your old name. Enter a new one.")
    elif is_present:
        await message.answer("This name already exists. Enter a unique one.")
    else:
